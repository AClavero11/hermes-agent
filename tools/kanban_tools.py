...
Worker lifecycle tools are only registered into the model's schema when the
agent is running under the dispatcher (env var ``HERMES_KANBAN_TASK`` set).
Safe orchestration tools (show/comment/create/link) may also be exposed to
normal chat sessions when ``kanban.orchestrator_tools`` is enabled in
``config.yaml``. That is what lets a chief-of-staff Telegram turn route work
to specialist profiles without shelling out.
...
1. **Backend portability.** A worker whose terminal tool points at Docker
   ...
   isn't mounted. Tools run in the agent's Python process, so they
    always reach the shared Hermes-root ``kanban.db`` regardless of terminal
    backend.
...
(``hermes dashboard``), and the slash command (``/kanban …``) — all three
bypass the agent entirely. The tools are for agent-side routing and worker
handoff back to the kernel.
"""
...
def _check_kanban_worker_mode() -> bool:
    """Worker-only tools are available inside dispatcher-spawned workers."""
    return bool(os.environ.get("HERMES_KANBAN_TASK"))


def _check_kanban_orchestrator_mode() -> bool:
    """Safe routing tools are available to workers and configured orchestrators."""
    if _check_kanban_worker_mode():
        return True
    if _flag(os.environ.get("HERMES_KANBAN_ORCHESTRATOR_TOOLS")):
        return True
    try:
        from hermes_cli.config import load_config

        cfg = load_config()
        return _flag((cfg.get("kanban") or {}).get("orchestrator_tools"))
    except Exception:
        return False


def _current_author() -> str:
    """Return a durable board author name for comments/tasks."""
    for env_name in ("HERMES_PROFILE", "HERMES_PROFILE_NAME"):
        value = os.environ.get(env_name)
        if value:
            return value
    return "worker" if _check_kanban_worker_mode() else "orchestrator"


def _origin_from_session_context() -> dict[str, Optional[str]]:
    """Return current gateway source context for task notifications."""
    try:
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = args.get("author") or _current_author()
    try:
        kb, conn = _connect()
        try:
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                skills=skills,
                created_by=_current_author(),
            )
            subscription = None
            origin = _origin_from_session_context()
            if origin:
                kb.add_notify_sub(
                    conn,
                    task_id=new_tid,
...
def _handle_workflow_create(args: dict, **kw) -> str:
    """Create a built-in specialist workflow DAG from one tool call."""
    template_id = args.get("template_id")
    title = args.get("title")
    if not template_id or not str(template_id).strip():
        return tool_error("template_id is required")
    if not title or not str(title).strip():
        return tool_error("title is required")
    priority = args.get("priority")
    workspace_kind = args.get("workspace_kind") or "scratch"
    ...
    try:
        ...
        try:
            ...
            if origin:
                ...
                subscribed = list(workflow.get("task_ids", []))
            return _ok(
                workflow=workflow,
        ...
        logger.exception("kanban_create_workflow failed")
        return tool_error(f"kanban_create_workflow: {e}")
...
KANBAN_WORKFLOW_CREATE_SCHEMA = {
    "name": "kanban_create_workflow",
    "description": (
        "Create a built-in multi-task workflow DAG for common executive-agent "
        "jobs. Prefer this over manual kanban_create calls when the request "
        "matches a known template: research-brief, operator-fix, rfq-package, "
        "or finance-follow-up. The tool creates specialist tasks, dependency "
        "links, workflow step metadata, and Telegram terminal-event "
        "subscriptions for the current chat when available."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "template_id": {
                ...
                "description": "Workflow template to instantiate.",
            },
            "title": {
                "type": "string",
                "description": "Workflow goal/title, e.g. 'Evaluate X links'.",
            },
            "body": {
                "type": "string",
                "description": (
                    "Shared context every workflow step receives: user request, "
                    "links, acceptance criteria, constraints, and approval gates."
                ),
            },
            "tenant": {
                "type": "string",
                "description": (
                    "Optional namespace for multi-project isolation. Defaults "
                    "to HERMES_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
                "description": "Base dispatcher priority; step deltas are added.",
            },
            "workspace_kind": {
                "type": "string",
                "enum": ["scratch", "dir", "worktree"],
                "description": "Workspace flavor shared by all workflow tasks.",
            },
            "workspace_path": {
                "type": "string",
                "description": "Absolute path for dir/worktree workflows.",
            },
            "idempotency_key": {
                "type": "string",
                "description": (
                    "Retry-safe dedup key prefix. Each step appends :<step-key>."
                ),
            },
        },
