...
def kanban_db_path() -> Path:
    """Return the shared board path under the root Hermes home.

    Profile workers run with ``HERMES_HOME=<root>/profiles/<name>``.
    The Kanban board must remain shared across those profiles, so anchor it
    at the default Hermes root rather than the active profile directory.
    """
...
def _default_spawn(task: Task, workspace: str) -> Optional[int]:
    ...
    if hermes_executable:
        cmd = [hermes_executable]
    else:
        cli_main = Path(__file__).resolve().parent / "main.py"
        cmd = [sys.executable, str(cli_main)]
    cmd.extend([
        "-p", task.assignee,
    ...
    # Redirect output to a per-task log under the shared root board home.
    from hermes_constants import get_default_hermes_root
    log_dir = get_default_hermes_root() / "kanban" / "logs"
...
def chief_of_staff_brief(conn: sqlite3.Connection, *, limit: int = 5) -> dict:
    """Return a compact board brief for Telegram/cron status checks."""
    try:
    ...
    buckets = {
        status: [
            _task_summary(task)
            for task in list_tasks(conn, status=status, limit=row_limit)
        ]
        for status in ("blocked", "running", "ready", "triage")
    }
    return {
...
def list_profiles_on_disk() -> list[str]:
    ...
    Reads ``<hermes-root>/profiles/`` so custom installs such as
    ``~/.hermes-deepseek`` and profile-mode workers all discover the same
    assignee pool. Only returns directories that contain a ``config.yaml``;
    a bare dir without config isn't a real profile.
    """
    try:
        from hermes_constants import get_default_hermes_root
