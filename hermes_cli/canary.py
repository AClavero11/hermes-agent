...
class CanaryOptions:
    ...
    approved_send_rehearsal: bool = False
...
def _is_internal_test_recipient(email: str) -> bool:
...
def _build_approved_quote_send_rehearsal_package(options: CanaryOptions) -> dict[str, Any]:
    ...
    draft_workspace = _attach_approved_rfq_draft_to_workspace(options, draft_package, draft_artifacts)
    draft_package["workspace"] = draft_workspace
    draft_artifacts = _write_approved_rfq_draft_artifacts(options, draft_package)

    source = draft_package.get("source_rfq_package") or {}
    scenario = source.get("scenario") or {}
    line = draft_package.get("draft_line") or {}
    customer_name = str(scenario.get("customer_name") or "Hermes Canary Customer")
    part_number = str(line.get("part_number") or scenario.get("part_number") or "")
    quantity = _float_or_zero(line.get("quantity")) or 1.0
    unit_price = _float_or_zero(line.get("unit_price"))
    subtotal = round(quantity * unit_price, 2)
    rehearsal_ref = f"HERMES-CANARY-SEND-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
    test_recipient = _approved_send_rehearsal_recipient()
    follow_up_due_at = (datetime.now(timezone.utc) + timedelta(days=3)).replace(microsecond=0).isoformat()
    subject = f"REHEARSAL ONLY - Quote {draft_package.get('draft_reference', rehearsal_ref)}"
    body = "\n".join(
        [
            "REHEARSAL ONLY - no customer send executed.",
            "",
            ...
            f"Total: ${subtotal:,.2f}",
            "",
            "Attached artifact is the QAMFORM preview generated by the guarded draft canary.",
            "Real customer delivery requires explicit operator approval after reviewing price, trace, and PDF.",
        ]
    )
    audit_events = [
        {
            "event_type": "quote_send.rehearsal_prepared",
            "created_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
            "actor": "hermes_canary",
            "reference": rehearsal_ref,
            "external_send": False,
        },
        {
            "event_type": "quote_send.customer_send_blocked",
            "created_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
            "actor": "hermes_canary",
            "reference": rehearsal_ref,
            "reason": "rehearsal_only_requires_explicit_operator_approval",
        },
        {
            "event_type": "quote_send.follow_up_scheduled_internal",
            "created_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
            "actor": "hermes_canary",
            "reference": rehearsal_ref,
    ...
    package = {
        ...
        "rehearsal_reference": rehearsal_ref,
        "source_draft_package": draft_package,
        "source_draft_artifacts": draft_artifacts,
        "operator_approval": {
            "status": "approved_for_send_rehearsal_only",
            "mode": "canary_simulated_operator_approval",
            "real_customer_send_approved": False,
            "approved_actions": [
                "prepare_internal_email_draft",
                "attach_qamform_preview",
                "write_audit_artifacts",
                "schedule_internal_follow_up",
                "surface_exception_plan",
            ],
            "blocked_actions": [
                "gmail_send",
                "smtp_send",
                "customer_email_delivery",
                "customer_telegram_delivery",
                "v11_quote_confirmation",
                "atlas_customer_send",
            ],
        },
        "email_draft": {
            "channel": "gmail_draft_rehearsal",
            "send_enabled": False,
            "external_delivery": False,
            "customer_recipient_present": False,
            "to": [test_recipient],
            "cc": [],
            "bcc": [],
            "subject": subject,
            ...
            "attachments": [
                {
                    "kind": "qamform_preview",
                    "path": draft_artifacts.get("pdf_path", ""),
                }
            ],
        },
        "audit": {
            ...
            "events": audit_events,
            "required_events": [
                "quote_send.rehearsal_prepared",
                "quote_send.customer_send_blocked",
                "quote_send.follow_up_scheduled_internal",
            ],
        },
        "follow_up": {
            "status": "scheduled_internal_rehearsal",
            "owner": "hermes",
            "due_at": follow_up_due_at,
            "action": "Review rehearsed quote-send packet before any real customer send approval.",
            "customer_facing": False,
        },
        "exception_surface": {
            "status": "prepared",
            "exceptions": [
                "missing_qamform_preview",
                "missing_internal_test_recipient",
                "customer_recipient_detected",
                "send_enabled_true",
...
def _render_approved_send_rehearsal_markdown(package: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Hermes Approved Quote Send Rehearsal",
            "",
            f"- Rehearsal reference: {package.get('rehearsal_reference', '')}",
            f"- Draft reference: {source.get('draft_reference', '')}",
            f"- Part: {draft_line.get('part_number', '')}",
            f"- Quantity: {_float_or_zero(draft_line.get('quantity')):g}",
            ...
            "## Guardrails",
            "",
            f"- Rehearsal only: {bool(guardrails.get('rehearsal_only'))}",
            f"- Customer email send: {bool(guardrails.get('customer_email_send'))}",
            f"- Gmail send: {bool(guardrails.get('gmail_send'))}",
            f"- Customer-facing send: {bool(guardrails.get('customer_facing_send'))}",
            f"- Approval required before real customer send: {bool(guardrails.get('approval_required_before_real_customer_send'))}",
            "",
            "## Email Draft Body",
            "",
            "```text",
            str(email_draft.get("body") or ""),
            "```",
            "",
        ]
    )
...
def _write_approved_send_rehearsal_artifacts(
    options: CanaryOptions,
    package: dict[str, Any],
) -> dict[str, str]:
    output_dir = options.hermes_home / "canary" / "approved_send_rehearsals"
    output_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    ref = str(package.get("rehearsal_reference") or "hermes-canary-send").lower()
    slug = "".join(ch if ch.isalnum() else "-" for ch in ref).strip("-")[:80]
    json_path = output_dir / f"{timestamp}-{slug}.json"
    markdown_path = output_dir / f"{timestamp}-{slug}.md"
    email_path = output_dir / f"{timestamp}-{slug}-email-draft.txt"
    audit_path = output_dir / f"{timestamp}-{slug}-audit.jsonl"
    email_draft = package.get("email_draft") or {}
    audit_events = (package.get("audit") or {}).get("events") or []
    json_path.write_text(json.dumps(package, indent=2, sort_keys=True), encoding="utf-8")
    markdown_path.write_text(_render_approved_send_rehearsal_markdown(package), encoding="utf-8")
    email_path.write_text(
        "\n".join(
            [
                f"To: {', '.join(email_draft.get('to') or [])}",
    ...
    latest_markdown = output_dir / "latest.md"
    latest_json.write_text(json.dumps(package, indent=2, sort_keys=True), encoding="utf-8")
    latest_markdown.write_text(_render_approved_send_rehearsal_markdown(package), encoding="utf-8")
    return {
        "json_path": str(json_path),
        "markdown_path": str(markdown_path),
        ...
        "latest_json": str(latest_json),
        "latest_markdown": str(latest_markdown),
        "pdf_path": str(((email_draft.get("attachments") or [{}])[0]).get("path", "")),
    }


def _attach_approved_send_rehearsal_to_workspace(
    options: CanaryOptions,
    package: dict[str, Any],
    artifacts: dict[str, str],
) -> dict[str, str]:
    ...
    store = WorkspaceStore(options.hermes_home / "workspace" / "control_plane.json")
    task = store.create_task(
        f"Approved quote-send rehearsal: {package.get('rehearsal_reference', '')}",
        owner="hermes",
        priority="normal",
        project="hermes-canary",
        source="canary",
        status="in_progress",
        note="Internal/test-recipient send rehearsal prepared; customer send remains blocked.",
    )
    store.add_evidence(
        task_id=task["id"],
        kind="approved_quote_send_rehearsal",
        title="Guarded approved quote-send rehearsal packet",
        locator=artifacts["json_path"],
        summary=(
            f"Email draft: {artifacts['email_path']}; audit: {artifacts['audit_path']}; "
            f"QAMFORM preview: {artifacts['pdf_path']}"
        ),
        metadata={
            "markdown_path": artifacts["markdown_path"],
            "email_path": artifacts["email_path"],
    ...
    store.update_task(
        task["id"],
        status="done",
        next_action="Real customer send remains blocked until explicit operator approval.",
    )
    return {"task_id": task["id"], "store_path": str(store.path)}
...
def _canary_approved_quote_send_rehearsal(options: CanaryOptions) -> CanaryResult:
    if not options.approved_send_rehearsal:
        return _result(
            ...
            0,
            40,
            "Approved quote-send rehearsal not enabled; pass --approved-send-rehearsal for internal send-packet proof",
            {"enabled": False},
        )

    try:
        package = _build_approved_quote_send_rehearsal_package(options)
        artifacts = _write_approved_send_rehearsal_artifacts(options, package)
        workspace = _attach_approved_send_rehearsal_to_workspace(options, package, artifacts)
        package["workspace"] = workspace
        artifacts = _write_approved_send_rehearsal_artifacts(options, package)
    except Exception as exc:
        return _result(
            ...
            0,
            40,
            f"Approved quote-send rehearsal failed: {type(exc).__name__}: {exc}",
            {
                "enabled": True,
                "failure_class": "approved_send_rehearsal_failure",
                "error_type": type(exc).__name__,
            },
        )
    ...
    required_events = set(audit.get("required_events") or [])
    attachment_path = str(((email_draft.get("attachments") or [{}])[0]).get("path", ""))
    checks = {
        "source_draft_package_loaded": bool(source.get("draft_reference")),
        "qamform_attachment_exists": bool(attachment_path)
        and Path(attachment_path).is_file()
        and Path(attachment_path).stat().st_size > 1000,
        "email_draft_ready": bool(email_draft.get("subject"))
        and "REHEARSAL ONLY" in str(email_draft.get("body") or "")
        and bool(to_recipients),
        "test_recipient_internal_only": bool(to_recipients)
        and all(_is_internal_test_recipient(item) for item in to_recipients),
        "customer_recipient_blocked": email_draft.get("customer_recipient_present") is False,
        "send_disabled": email_draft.get("send_enabled") is False
        and email_draft.get("external_delivery") is False
        and guardrails.get("customer_email_send") is False
        and guardrails.get("gmail_send") is False
        and guardrails.get("smtp_send") is False,
        "business_writes_blocked": guardrails.get("v11_create_or_write") is False
        and guardrails.get("atlas_create_or_write") is False
        and guardrails.get("write_methods_called") == [],
        "real_send_requires_approval": guardrails.get("approval_required_before_real_customer_send") is True
        and (package.get("operator_approval") or {}).get("real_customer_send_approved") is False,
        "audit_events_written": required_events.issubset(event_types)
        and Path(artifacts["audit_path"]).is_file(),
        "proof_artifacts_written": all(
            Path(artifacts[name]).is_file()
            for name in ("json_path", "markdown_path", "email_path", "audit_path")
        ),
        "follow_up_scheduled": follow_up.get("status") == "scheduled_internal_rehearsal"
        and bool(follow_up.get("due_at"))
    ...
    failed = {name: value for name, value in checks.items() if not value}
    score = round(40.0 * (len(checks) - len(failed)) / len(checks), 1)
    status = PASS if not failed else WARN
    return _result(
        "live.approved_quote_send_rehearsal",
        status,
        score,
        40,
        f"{len(checks) - len(failed)}/{len(checks)} approved quote-send rehearsal checks passed",
        {
            "enabled": True,
            "checks": checks,
            "failed": failed,
...
def _quality_dimensions(report: CanaryReport) -> list[QualityDimension]:
    ...
    if (
        ...
        business_ops_score = 9.8
        business_ops_summary = "Approved-send rehearsal prepares internal email draft, audit proof, follow-up, and exception surface with customer sends blocked."
    elif (
        quote_ops_result
        and quote_ops_result.status == PASS
        and rfq_dry_run_result
...
def quality_summary(report: CanaryReport) -> dict[str, Any]:
    ...
    increments = [
        ...
        {
            "target": "10.0/10",
            "increment": "Real approved quotes send to customers, write audit proof, schedule follow-up, and surface exceptions",
            "status": "open",
        },
    ]
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=os.getenv("HERMES_CANARY_APPROVED_SEND_REHEARSAL", "").strip().lower()
        in {"1", "true", "yes", "on"},
        help=(
            "Build an approved quote-send rehearsal packet with internal/test recipient, "
            "audit proof, follow-up schedule, and customer sends disabled"
        ),
    )
    parser.add_argument(
        "--api-key",
...
def options_from_args(args: argparse.Namespace) -> CanaryOptions:
    ...
    return CanaryOptions(
        ...
        approved_send_rehearsal=bool(getattr(args, "approved_send_rehearsal", False)),
