...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_brief = sub.add_parser(
        "brief",
        help="Chief-of-staff board brief: blockers, running work, ready queue, triage",
    )
    p_brief.add_argument("--limit", type=int, default=5,
                         help="Max tasks per status bucket (default: 5)")
    p_brief.add_argument("--json", action="store_true")

    # --- workflow templates ---
    p_workflow = sub.add_parser(
        "workflow",
        help="Create specialist-task DAGs from built-in workflow templates",
    )
    workflow_sub = p_workflow.add_subparsers(dest="workflow_action")

    p_wf_list = workflow_sub.add_parser(
        "list",
        help="List available workflow templates",
    )
    p_wf_list.add_argument("--json", action="store_true")

    p_wf_create = workflow_sub.add_parser(
        "create",
        help="Create all tasks for a workflow template",
    )
    p_wf_create.add_argument("template_id", help="Template id, e.g. research-brief")
    p_wf_create.add_argument("--title", required=True, help="Workflow goal/title")
    p_wf_create.add_argument("--body", default=None, help="Shared workflow context")
    p_wf_create.add_argument("--workspace", default="scratch",
                             help="scratch | worktree | dir:<path> (default: scratch)")
    p_wf_create.add_argument("--tenant", default=None, help="Tenant namespace")
    p_wf_create.add_argument("--priority", type=int, default=0,
                             help="Base priority; step deltas are added")
    p_wf_create.add_argument("--idempotency-key", default=None,
                             help="Dedup key prefix; each step appends :<step-key>")
    p_wf_create.add_argument("--created-by", default=None,
                             help="Author recorded on each task")
    p_wf_create.add_argument("--json", action="store_true")
...
def _cmd_brief(args: argparse.Namespace) -> int:
    with kb.connect() as conn:
        # Same cheap promotion pass as list so the brief reflects newly
        # unblocked dependencies before it reports the ready queue.
        kb.recompute_ready(conn)
        brief = kb.chief_of_staff_brief(conn, limit=getattr(args, "limit", 5))
    if getattr(args, "json", False):
    ...
    generated = _fmt_ts(int(brief.get("generated_at") or 0))
    print(f"Chief-of-staff brief ({generated})")
    print(
        "Status: "
        + ", ".join(
    ...
    if age is not None:
        print(f"Oldest ready age: {int(age)}s")
    ...
    for status, label in (
        ...
        tasks = (brief.get("buckets") or {}).get(status) or []
        print()
        print(f"{label}:")
        if not tasks:
            print("  (none)")
        ...
        for task in tasks:
            ...
            tenant = f" [{task['tenant']}]" if task.get("tenant") else ""
            workflow = ""
            if task.get("workflow_template_id"):
                workflow = (
                    f" ({task['workflow_template_id']}:"
                    f"{task.get('current_step_key') or '-'})"
                )
            print(
                f"  {task['id']} @{assignee}{tenant}{workflow}: {task['title']}"
            )
    return 0
...
def _cmd_workflow(args: argparse.Namespace) -> int:
    ...
    if not action:
        templates = kwf.list_templates()
        for template in templates:
            print(f"{template['id']:18s}  {template['name']} - {template['description']}")
        return 0
    ...
    if action == "list":
        ...
        if getattr(args, "json", False):
            ...
            return 0
        for template in templates:
            print(f"{template['id']:18s}  {template['name']}")
            print(f"  {template['description']}")
            steps = ", ".join(
                f"{step['key']}@{step['assignee']}"
    ...
    if action == "create":
        ...
        if getattr(args, "json", False):
            ...
            return 0
        print(f"Created workflow {workflow['template_id']}: {workflow['title']}")
        for task in workflow.get("tasks", []):
            parents = (
                f" parents={','.join(task['parents'])}"
                if task.get("parents") else ""
            )
            print(
                f"  {task['key']:18s} {task['task_id']} "
                f"{task['status']:8s} @{task['assignee']}{parents}"
            )
        return 0
