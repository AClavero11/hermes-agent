...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    """list_profiles_on_disk returns profile dirs under the Hermes root."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_kanban_paths_share_root_in_profile_mode(tmp_path, monkeypatch):
    """Profile workers use the root board, not profile-local kanban state."""
    root = tmp_path / ".hermes-deepseek"
...
def test_pid_alive_detects_zombie(kanban_home):
    ...
    try:
        ...
        with open(f"/proc/{pid}/status") as f:
            state_line = next(
                (line for line in f if line.startswith("State:")), ""
            )
...
def test_default_spawn_falls_back_to_python_entrypoint(kanban_home, monkeypatch):
    """Workers still spawn when launchd PATH cannot resolve `hermes`."""
    captured = {}
    ...
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="path fallback", assignee="operator")
        task = kb.get_task(conn, tid)
        workspace = kb.resolve_workspace(task)
        pid = kb._default_spawn(task, str(workspace))
        assert pid == 12345
    ...
    assert cmd[1].endswith("hermes_cli/main.py")
    assert cmd[2:4] == ["-p", "operator"]
