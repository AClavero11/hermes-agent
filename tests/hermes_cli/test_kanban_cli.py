...
def test_run_slash_workflow_list_and_create_json(kanban_home):
    ...
    out = kc.run_slash(
        ...
        "--body 'Summarize the two X posts' --json"
    )
    payload = json.loads(out)
    assert payload["template_id"] == "research-brief"
    assert len(payload["tasks"]) == 2
    assert payload["tasks"][0]["status"] == "ready"
    assert payload["tasks"][1]["status"] == "todo"

    with kb.connect() as conn:
        first = kb.get_task(conn, payload["tasks"][0]["task_id"])
        second = kb.get_task(conn, payload["tasks"][1]["task_id"])
        assert first.workflow_template_id == "research-brief"
...
def test_run_slash_brief_json(kanban_home):
    kc.run_slash("create 'blocked item' --assignee operator")
    import re
    tid = re.search(r"(t_[a-f0-9]+)", kc.run_slash("list")).group(1)
    kc.run_slash(f"block {tid} 'need approval'")
    payload = json.loads(kc.run_slash("brief --json"))
    assert payload["stats"]["by_status"]["blocked"] == 1
    assert payload["buckets"]["blocked"][0]["title"] == "blocked item"
