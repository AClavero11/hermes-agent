...
Verifies:
  - Lifecycle tools are gated on HERMES_KANBAN_TASK, while safe routing
    tools can be enabled for orchestrator sessions by config.
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    ...
    assert kanban_tools is not None
...
def test_kanban_orchestrator_tools_visible_with_config(monkeypatch, tmp_path):
    """Configured orchestrator sessions can route work without worker-only verbs."""
    monkeypatch.delenv("HERMES_KANBAN_TASK", raising=False)
    home = tmp_path / ".hermes"
    home.mkdir()
    ...
    kanban = {n for n in names if n and n.startswith("kanban_")}
    expected = {
        "kanban_show", "kanban_comment", "kanban_create",
        "kanban_create_workflow", "kanban_link",
    }
    assert kanban == expected, f"expected {expected}, got {kanban}"
...
def test_create_auto_subscribes_gateway_origin(worker_env):
    """Natural-language Telegram routing should receive task terminal updates."""
    ...
    d = json.loads(out)
    assert d["ok"] is True
    assert d["subscribed"]["platform"] == "telegram"
    assert d["subscribed"]["chat_id"] == "496461229"

    with kb.connect() as conn:
        subs = kb.list_notify_subs(conn, d["task_id"])
    assert len(subs) == 1
    assert subs[0]["platform"] == "telegram"
    assert subs[0]["chat_id"] == "496461229"
    assert subs[0]["thread_id"] == "123"
    assert subs[0]["user_id"] == "42"
...
def test_create_workflow_happy_path(worker_env):
    from tools import kanban_tools as kt
    from hermes_cli import kanban_db as kb

    out = kt._handle_workflow_create({
        "template_id": "operator-fix",
        "title": "Repair gateway routing",
    ...
    with kb.connect() as conn:
        first = kb.get_task(conn, d["task_ids"][0])
        second = kb.get_task(conn, d["task_ids"][1])
        assert first.assignee == "operator"
        assert first.workflow_template_id == "operator-fix"
        assert first.current_step_key == "diagnose-fix"
        assert second.assignee == "chief-of-staff"
        assert second.status == "todo"
        assert first.id in kb.parent_ids(conn, second.id)


def test_create_workflow_rejects_unknown_template(worker_env):
    from tools import kanban_tools as kt
    out = kt._handle_workflow_create({
...
def test_kanban_orchestrator_guidance_in_prompt(monkeypatch, tmp_path):
    """A configured non-worker orchestrator gets routing guidance, not worker lifecycle."""
    monkeypatch.delenv("HERMES_KANBAN_TASK", raising=False)
    home = tmp_path / ".hermes"
    home.mkdir()
    (home / "config.yaml").write_text(
    ...
    prompt = a._build_system_prompt()
    assert "You are a Kanban orchestrator" in prompt
    assert "You are a Kanban worker" not in prompt
    assert "chief-of-staff" in prompt
    assert "kanban_create" in prompt
    assert "kanban_create_workflow" in prompt
